...
class BudgetForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "total_expense_limit": forms.NumberInput(
                attrs={"class": "form-control", "step": "0.01"}
            ),
        }
...
class BudgetCategoryLimitFormSet(BaseInlineFormSet):
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        if user:
            self.category_queryset = Category.objects.filter(
                Q(is_system=True) | Q(user=user), type=Type.OUTCOME
            ).order_by("name")
        else:
            self.category_queryset = Category.objects.filter(
                type=Type.OUTCOME
        ...
        for form in self.forms:
            form.fields["category"].queryset = self.category_queryset
        self.empty_form.fields["category"].queryset = self.category_queryset

    def _construct_form(self, i, **kwargs):
        form = super()._construct_form(i, **kwargs)
        if hasattr(self, "category_queryset"):
            form.fields["category"].queryset = self.category_queryset
        return form
