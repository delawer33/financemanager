...
class AccountDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        income_total = transactions.filter(type=Type.INCOME).aggregate(Sum("amount"))[
            "amount__sum"
        ] or Decimal("0")
        outcome_total = transactions.filter(type=Type.OUTCOME).aggregate(Sum("amount"))[
            "amount__sum"
        ] or Decimal("0")
...
class BudgetCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.method == "POST":
            budget_instance = (
                ...
                and self.object
                or Budget(user=self.request.user)
            )
            ...
            budget_instance = Budget(user=self.request.user)
            context["category_limits"] = BudgetCategoryLimitFormSet(
                instance=budget_instance,
                prefix="category_limits",
                user=self.request.user,
        ...
        #     Q(is_system=True) | Q(user=self.request.user), type=Type.OUTCOME
        # ).order_by("name")
        # print(context["category_limits"])
        return context
    ...
    def form_valid(self, form):
        ...
        # else:
        #     category_limits = BudgetCategoryLimitFormSet(
        # instance = self.object, prefix = "category_limits", user = self.request.user
        #     )
...
class BudgetDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for limit in category_limits:
            ...
            percentage = (
                (spent / limit.limit_amount * 100) if limit.limit_amount > 0 else 0
            )
        ...
        transactions = (
            Transaction.objects.filter(
                user=self.request.user, date__range=[budget.start_date, budget.end_date]
            )
        ...
        context.update(
            {
                ...
                if budget.total_expense_limit and budget.total_expense_limit > 0
                else 0,
            }
        )
...
def transaction_delete(request, pk):
    ...
    return render(
        request, "transaction/transaction_list_part.html", {"list_filter": list_filter}
    )
...
def get_categories_by_type(request):
    ...
    return render(
        request, "transaction/category_dropdown.html", {"categories": user_categories}
    )
...
def category_delete(request, pk):
    ...
    if request.method == "POST":
        cat.delete()
        messages.success(
            request, f'Category "{cat.translated_name}" deleted successfully!'
        )

    categories = Category.objects.filter(Q(is_system=True) | Q(user=request.user))
    return render(request, "transaction/category_list.html", {"categories": categories})
...
def recur_transaction_delete(request, pk):
    ...
    qs = RecurringTransaction.objects.filter(user=request.user).order_by("-id")

    return render(
        request, "transaction/recur_trans_list.html", {"recur_transactions": qs}
    )
